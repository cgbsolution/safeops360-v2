...
export function TemplateBuilder({ template, clauses, perms }: { template: TemplateDetail; clauses: ClauseRef[]; perms: { author: boolean; approve: boolean } }) {
  ...
  return (
    <div className="space-y-4">
      <Card className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-none">
      ...
      {!editable && template.status === "APPROVED" && (
        <Alert variant="success" className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-800">This version is approved and immutable. Clone it to make edits — a new draft version preserves this snapshot.</Alert>
      )}

      {sections.map((s, si) => (
        <Card key={si} className="rounded-xl border border-slate-200 bg-white shadow-none">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="min-w-0 flex-1 space-y-2">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <SelectField disabled={!editable} value={q.questionType} onChange={(value) => patchQuestion(si, qi, { questionType: value })} className="rounded border border-slate-200 px-1.5 py-1 text-xs"
                        options={QUESTION_TYPES.map((t) => ({ value: t.value, label: t.label }))}
                      />
                      <SelectField disabled={!editable} value={q.standardClauseRef ?? ""} onChange={(value) => patchQuestion(si, qi, { standardClauseRef: value || null })} className="rounded border border-indigo-200 bg-indigo-50/50 px-1.5 py-1 text-xs text-indigo-700"
                        placeholder="— clause —"
                        options={clauses.map((c) => ({ value: c.clause, label: `${c.clause} · ${c.title}` }))}
                      />
