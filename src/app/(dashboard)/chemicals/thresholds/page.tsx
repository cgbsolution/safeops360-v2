...
import {
  Chip, EmptyState, ErrorState, Kpi, StatusChip, SubNav, ThresholdBar, TILE,
} from "../_components";
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <Link
            href={`/chemicals/thresholds?plantId=${ctx.plantId}&recompute=1`}
                  ...
                  {rows.map((r) => (
                    <tr key={r.ruleId} className={r.status === "BREACHED" ? "bg-rose-50/40" : "hover:bg-slate-50"}>
                      ...
                      <td className="px-4 py-2.5">
                        ...
                        {!r.autoMocOnBreach && (
                          <Chip label="Auto-MOC off" tone="bg-slate-100 text-slate-600 border-slate-300" />
                        )}
                        {r.activeMocId && (
                          <Link
