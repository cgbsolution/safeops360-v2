...
//     scheduled & planned; 4 raised by consumer modules (Fire / PPE / Paper Machine /
...
//     5 repeat findings at South Kraft & Board Mill (ISO 45001 clauses 8.1.2/8.2/6.1.2/9.1
...
async function main() {
  ...
  const etpTpl = await approvedTemplate({
    ...
    sections: [
      { title: "Compliance Obligations", questions: [
        ...
        { text: "Is bleach-plant effluent within consented norms for COD/BOD/AOX/colour (ETP logs verified)?", clause: "ISO 14001:9.1.2" },
      ]},
      { title: "Operational Control", questions: [
        { text: "Is the ETP operated and maintained per SOP for the pulp & paper effluent stream?", clause: "ISO 14001:8.1" },
        { text: "Is emergency response for black-liquor / chemical spill release in place?", clause: "ISO 14001:8.2" },
  ...
  const qmsTpl = await approvedTemplate({
    ...
    sections: [
      { title: "Production Control", questions: [
        { text: "Is paper-machine production controlled per plan for the target grade and GSM?", clause: "ISO 9001:8.5.1" },
        { text: "Are monitoring & measuring resources (grammage, moisture, brightness gauges) calibrated?", clause: "ISO 9001:7.1.5" },
        { text: "Are nonconforming reels/reams controlled and segregated as broke?", clause: "ISO 9001:8.7" },
      ]},
  ...
  await engagement({ title: "Internal HSE System Audit — North Paper Mill FY26", type: "INTERNAL_AUDIT", auditTypeId: atHse.id, templateId: hseTpl.id, standards: ["ISO_45001"], plant: nw, status: "CLOSED", lead: leadAuditor, plannedAgo: 150, score: 88, result: "MINOR_NC",
  ...
  // FY26-Q3 BASELINE at South Kraft & Board Mill — the "first occurrences" the later audits recur against.
  await engagement({ title: "Internal HSE System Audit — South Kraft & Board Mill FY26-Q3 (baseline)", type: "INTERNAL_AUDIT", auditTypeId: atHse.id, templateId: hseTpl.id, standards: ["ISO_45001"], plant: sw, status: "CLOSED", lead: leadAuditor, plannedAgo: 210, score: 74, result: "MAJOR_NC",
    findings: [
      { title: "Machine guarding gap first identified at rewinder line", severity: "MAJOR_NC", clause: "ISO 45001:8.1.2", status: "CLOSED", capaState: "CLOSED", ageDays: 205, rca: "Nip-guard interlock missing on retrofit rewinder." },
      { title: "Emergency drill not conducted in Q3", severity: "MINOR_NC", clause: "ISO 45001:8.2", status: "CLOSED" },
      { title: "Hazard identification not refreshed for new rewinding process", severity: "MAJOR_NC", clause: "ISO 45001:6.1.2", status: "CLOSED", capaState: "CLOSED", ageDays: 205, rca: "MOC not triggered for the rewinder line change." },
      { title: "OH&S performance metrics not reviewed in Q3", severity: "MINOR_NC", clause: "ISO 45001:9.1", status: "CLOSED" },
    ] });

  await engagement({ title: "Internal HSE System Audit — South Kraft & Board Mill FY26", type: "INTERNAL_AUDIT", auditTypeId: atHse.id, templateId: hseTpl.id, standards: ["ISO_45001"], plant: sw, status: "CLOSED", lead: leadAuditor, plannedAgo: 140, score: 79, result: "MAJOR_NC",
    findings: [
      { title: "Hierarchy of controls not applied at rewinder line guarding", severity: "MAJOR_NC", clause: "ISO 45001:8.1.2", status: "CAPA_RAISED", capaState: "ACTIONS_IN_PROGRESS", repeat: true, ageDays: 130, rca: "Rewinder nip-guard upgrade deferred for budget." },
      { title: "Emergency drill overdue at South Kraft & Board Mill", severity: "MINOR_NC", clause: "ISO 45001:8.2", status: "CLOSED", repeat: true },
    ] });

  // ETP South Kraft & Board Mill — MAJOR_NC tying to the water/consent risk story.
  await engagement({ title: "ETP Compliance Audit — South Kraft & Board Mill", type: "COMPLIANCE_AUDIT", auditTypeId: atEtp.id, templateId: etpTpl.id, standards: ["ISO_14001"], plant: sw, status: "REPORT_ISSUED", lead: leadAuditor, plannedAgo: 45, score: 72, result: "MAJOR_NC",
    findings: [{ title: "Bleach-plant effluent COD/colour exceeded consented norm on 3 days", severity: "MAJOR_NC", clause: "ISO 14001:9.1.2", status: "CAPA_RAISED", capaState: "ACTIONS_PLANNED", ageDays: 40, rca: "ETP dosing pump under-capacity during peak black-liquor load." }] });

  // Quality system audit — North, clean.
  await engagement({ title: "Quality System Audit — North Paper Mill", type: "INTERNAL_AUDIT", auditTypeId: atQms.id, templateId: qmsTpl.id, standards: ["ISO_9001"], plant: nw, status: "CLOSED", lead: leadAuditor, plannedAgo: 95, score: 91, result: "CONFORMING" });

  // Recurrences — same ISO 45001 clauses recurring at South Kraft & Board Mill across quarters.
  ...
  await engagement({ title: "Internal HSE System Audit — South Kraft & Board Mill Q2", type: "INTERNAL_AUDIT", auditTypeId: atHse.id, templateId: hseTpl.id, standards: ["ISO_45001"], plant: sw, status: "REPORT_ISSUED", lead: leadAuditor, plannedAgo: 30, score: 77, result: "MAJOR_NC",
    findings: [{ title: "Machine guarding gap recurs at rewinder line (repeat)", severity: "MAJOR_NC", clause: "ISO 45001:8.1.2", status: "CAPA_RAISED", capaState: "ACTIONS_PLANNED", repeat: true, ageDays: 25, rca: "Prior CAPA not yet effective — interim guard only." }] });

  await engagement({ title: "Internal HSE System Audit — South Kraft & Board Mill FY27-Q1", type: "INTERNAL_AUDIT", auditTypeId: atHse.id, templateId: hseTpl.id, standards: ["ISO_45001"], plant: sw, status: "REPORT_ISSUED", lead: leadAuditor, plannedAgo: 25, score: 78, result: "MAJOR_NC",
    findings: [
      { title: "Hazard identification still not refreshed for rewinding process (repeat)", severity: "MAJOR_NC", clause: "ISO 45001:6.1.2", status: "CAPA_RAISED", capaState: "ACTIONS_PLANNED", repeat: true, ageDays: 20, rca: "Q3 corrective action not yet effective." },
  ...
  await engagement({ title: "Fire Equipment Inspection — North Paper Mill (June)", type: "INSPECTION", auditTypeId: atFire.id, templateId: fireTpl.id, standards: ["ISO_45001"], plant: nw, status: "CLOSED", lead: auditor, plannedAgo: 12, score: 100, result: "CONFORMING", sourceModule: "Fire Safety" });
  await engagement({ title: "PPE Condition Inspection — South Kraft & Board Mill Q2", type: "INSPECTION", auditTypeId: atPpe.id, templateId: null, standards: ["ISO_45001"], plant: sw, status: "FIELDWORK_COMPLETE", lead: auditor, plannedAgo: 6, score: 94, result: "MINOR_NC", sourceModule: "PPE Management",
    findings: [{ title: "3 hard hats past replacement date in store B", severity: "MINOR_NC", status: "OPEN" }] });
  await engagement({ title: "Grade-Change Line Clearance Audit — Paper Machine PM-2 (Reel 2026-118)", type: "INSPECTION", auditTypeId: atQms.id, templateId: qmsTpl.id, standards: ["ISO_9001"], plant: nw, status: "CLOSED", lead: auditor, plannedAgo: 3, score: 100, result: "CONFORMING", sourceModule: "Paper Machine IMS" });

  // In-progress / scheduled / planned (calendar forward fill).
  await engagement({ title: "Environmental System Audit — North Paper Mill", type: "INTERNAL_AUDIT", auditTypeId: atEnv.id, templateId: etpTpl.id, standards: ["ISO_14001"], plant: nw, status: "IN_PROGRESS", lead: leadAuditor, plannedAgo: 2 });
  ...
  await engagement({ title: "Fire Equipment Inspection — South Kraft & Board Mill (July)", type: "INSPECTION", auditTypeId: atFire.id, templateId: fireTpl.id, standards: ["ISO_45001"], plant: sw, status: "SCHEDULED", lead: auditor, plannedIn: 16 });
  await engagement({ title: "Statutory Compliance Audit — North Paper Mill FY27", type: "COMPLIANCE_AUDIT", auditTypeId: atStat.id, templateId: etpTpl.id, standards: ["ISO_45001", "ISO_14001"], plant: nw, status: "PLANNED", lead: leadAuditor, plannedIn: 40 });
  await engagement({ title: "Quality System Audit — South Kraft & Board Mill FY27", type: "INTERNAL_AUDIT", auditTypeId: atQms.id, templateId: qmsTpl.id, standards: ["ISO_9001"], plant: sw, status: "PLANNED", lead: leadAuditor, plannedIn: 55 });
  ...
  if (obligations.length) {
    const etp = await prisma.camsEngagement.findFirst({ where: { title: { contains: "ETP Compliance Audit — South Kraft & Board Mill" } } });
    const etpFinding = etp ? await prisma.camsFinding.findFirst({ where: { engagementId: etp.id } }) : null;
    const hseNorth = await prisma.camsEngagement.findFirst({ where: { title: { contains: "Internal HSE System Audit — North Paper Mill" } } });
    const qmsNorth = await prisma.camsEngagement.findFirst({ where: { title: { contains: "Quality System Audit — North Paper Mill" } } });
  ...
  await prisma.camsAssetLite.createMany({
    data: [
      ...
      { assetCode: "AST-SW-PKG01", name: "Rewinder Line 3", category: "PRODUCTION", siteId: sw.id, location: "Finishing House", createdBy: auditMgr },
    ],
  });
