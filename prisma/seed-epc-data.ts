...
async function main() {
  ...
  const siteNW = await prisma.constructionSite.create({
    data: {
      ...
      siteName: "Page Industries — North Paper Mill — Utilities Expansion Block C",
      ...
      address: "North Paper Mill Campus, Block C Extension, Andheri East, Mumbai",
  ...
  const siteSW = await prisma.constructionSite.create({
    data: {
      ...
      siteName: "Page Industries — South Kraft & Board Mill — New Paper Machine PM-3 Erection",
      ...
      address: "South Kraft & Board Mill Campus, Plot 7A, Taloja Industrial Area, Navi Mumbai",
      district: "Raigad",
      state: "Maharashtra",
      lat: 19.0456,
      lng: 73.1189,
      ...
      scopeDescription:
        "Greenfield erection of new Paper Machine PM-3 (duplex/packaging board line) comprising: machine hall building, stock preparation and approach flow, Fourdrinier wire & press section, multi-cylinder dryer section, size press and reel, rewinder, process utilities, ETP capacity addition, and supporting infrastructure.",
  ...
  for (const { id: workerId, spec, siteId } of workers) {
    ...
    const mob = await prisma.mobilizationRecord.create({
      data: {
        ...
        workArea: isNW ? "Block C Civil Works" : "PM-3 Machine Hall — Structural",
    ...
    const induction = await prisma.siteInduction.create({
      data: {
        ...
        topicsCovered: [
          ...
          isNW ? "Confined space hazards in Block C" : "Chemical and rotating-machinery hazards in PM-3 hall",
        ],
    ...
    const gatePass = await prisma.gatePass.create({
      data: {
        ...
        authorizedAreas: isNW ? ["Block C Civil", "Laydown Area", "Site Office"] : ["PM-3 Machine Hall Structure", "Laydown Yard", "Site Office"],
