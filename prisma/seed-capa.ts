  ...
  const CAPAS = [
    {
      ...
      title: "Bleaching-agent vapour release in dye house — corrective + preventive actions",
      sourceTypeCode: "SAFETY_INCIDENT", sourceCatCode: "SAFETY", subCatCode: "EQUIPMENT",
      actionType: "CORRECTIVE_AND_PREVENTIVE",
      problem: "Bleaching-agent (hydrogen peroxide / scouring chemical) vapour detected above TLV at the dye-house chemical dosing station. Corrosion on isolation valve stem allowed a micro-leak during peak load. Two dye-house operators exposed; first aid required.",
      detectedAt: daysAgo(90), rcaMethodology: "5_WHY",
      rcaSummary: "Root cause: isolation valve stem corrosion not identified during 6-monthly inspection due to absence of UT thickness testing on valve bodies. Contributing: no specific checklist item for CI valve stems in bleaching-chemical service.",
      ...
      actions: [
        { type: "IMMEDIATE_CONTAINMENT", desc: "Isolated valve, vented line, evacuated the dye-house dosing area. Air quality monitoring confirmed clearance before re-entry.", owner: maintHead.id, status: "COMPLETED", daysFromDetect: 1 },
        { type: "CORRECTIVE", desc: "Replace all CI isolation valves in bleaching-chemical service with corrosion-resistant titanium-lined valves. UT thickness test prior to re-commissioning.", owner: maintHead.id, status: "COMPLETED", daysFromDetect: 30 },
        { type: "PREVENTIVE", desc: "Revise preventive maintenance checklist to include UT thickness measurement for all valves in bleaching-chemical and other corrosive dye-house service. Frequency: quarterly.", owner: safetyOfficer.id, status: "COMPLETED", daysFromDetect: 45 },
      ],
      rootCauses: [
        { desc: "6-monthly visual inspection did not include ultrasonic wall thickness measurement on valve stems in corrosive dye-house service.", cat: "PROCESS", confidence: "HIGH" },
        { desc: "PM checklist had no specific line item for CI valve stem integrity in bleaching-chemical dosing service.", cat: "DOCUMENTATION", confidence: "HIGH" },
      ],
    },
    {
      num: "002", severity: "MODERATE", state: "VERIFIED",
      title: "Fabric-roll handling crane wire rope wear — inspection finding CAPA",
      sourceTypeCode: "INSPECTION_FINDING", sourceCatCode: "SAFETY", subCatCode: "EQUIPMENT",
      actionType: "CORRECTIVE_AND_PREVENTIVE",
      problem: "Monthly inspection of the dye-house fabric-roll handling EOT crane found wire rope with 3 broken wires in one lay, exceeding the 2-wire discard criterion. Crane taken out of service immediately.",
      ...
      rootCauses: [
        { desc: "Preventive lubrication schedule not tracked in CMMS — manual schedule missed after technician absence.", cat: "PROCESS", confidence: "HIGH" },
      ],
    },
    {
      ...
      actionType: "CORRECTIVE_AND_PREVENTIVE",
      problem: "Electrician began work on the knitting-section drive MCC panel without verifying de-energisation. Arc flash occurred; PPE prevented injury. Near-miss escalated to CAPA given high potential severity.",
    ...
    },
    {
      ...
      actionType: "CORRECTIVE_AND_PREVENTIVE",
      problem: "Internal audit (IMS-AUD-2026-Q1) identified that 3 of 5 dye-bath chemical handling operators could not locate the current revision of SOP-CHEM-005. One operator using superseded procedure.",
      detectedAt: daysAgo(21), rcaMethodology: "IS_IS_NOT",
      rcaSummary: "IS: Chemical storage & handling area, Shift B operators. IS NOT: Shift A, other areas. Documents issue: SOP revisions distributed via email only; operators on shift B had not accessed intranet in > 3 months. Root cause: document distribution system does not enforce acknowledgement for controlled procedures.",
      verificationResult: null,
    ...
    },
    {
      ...
      title: "Housekeeping deficiency — slip hazard in fabric & lint waste store",
      sourceTypeCode: "SAFETY_OBSERVATION", sourceCatCode: "SAFETY", subCatCode: "ENVIRONMENTAL",
      actionType: "CORRECTIVE_ONLY",
      problem: "Safety walk identified spilled loose lint and fabric cut-waste on the access aisle in the fabric & lint waste store. Aisle not cleaned at end of shift as per housekeeping standard.",
      ...
      actions: [
        { type: "CORRECTIVE", desc: "Clear spilled lint and fabric cut-waste; inspect and clean all access aisles in the fabric & lint waste store. Verify no other contamination.", owner: supervisor.id, status: "PROPOSED", daysFromDetect: 1 },
        { type: "CORRECTIVE", desc: "Reinforce end-of-shift housekeeping checklist for the fabric & lint waste store in next team briefing. Supervisor to verify completion daily for 30 days.", owner: supervisor.id, status: "PROPOSED", daysFromDetect: 7 },
      ],
      rootCauses: [
    ...
    },
    {
      ...
      title: "HIRA recommended control — high noise at knitting hall & compressor room",
      sourceTypeCode: "HIRA_CONTROL", sourceCatCode: "SAFETY", subCatCode: "ENVIRONMENTAL",
      actionType: "PREVENTIVE_ONLY",
      problem: "HIRA review of the knitting hall & compressor room identified noise level averaging 95 dB(A) — exceeding the 85 dB action level. Engineering noise control not yet installed. Hearing protection mandatory interim control only.",
      ...
      actions: [
        { type: "PREVENTIVE", desc: "Procure and install acoustic enclosures around air compressor units 1–3. Target: reduce noise at operator station to < 80 dB(A). Engineering evaluation to specify design.", owner: maintHead.id, status: "PROPOSED", daysFromDetect: 60 },
      ],
      rootCauses: [],
    },
  ];
  ...
  for (const c of CAPAS) {
    ...
    const capa = await prisma.capa.create({
      data: {
        ...
        affectedAreas: ["Dye House — Chemical Dosing Area", "Knitting Hall & Compressor Room"],
        affectedProcesses: ["Dyeing & Bleaching", "Equipment Maintenance"],
      },
    });
