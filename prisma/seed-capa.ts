  ...
  const CAPAS = [
    {
      ...
      title: "Chlorine dioxide release at bleach plant — corrective + preventive actions",
      sourceTypeCode: "SAFETY_INCIDENT", sourceCatCode: "SAFETY", subCatCode: "EQUIPMENT",
      actionType: "CORRECTIVE_AND_PREVENTIVE",
      problem: "Chlorine dioxide (ClO₂) detected above TLV at the D0 bleaching stage dosing station. Corrosion on isolation valve stem allowed a micro-leak during peak load. Two bleach plant operators exposed; first aid required.",
      detectedAt: daysAgo(90), rcaMethodology: "5_WHY",
      rcaSummary: "Root cause: isolation valve stem corrosion not identified during 6-monthly inspection due to absence of UT thickness testing on valve bodies. Contributing: no specific checklist item for CI valve stems in ClO₂ service.",
      ...
      actions: [
        { type: "IMMEDIATE_CONTAINMENT", desc: "Isolated valve, vented line, evacuated the bleach plant area. Air quality monitoring confirmed clearance before re-entry.", owner: maintHead.id, status: "COMPLETED", daysFromDetect: 1 },
        { type: "CORRECTIVE", desc: "Replace all CI isolation valves in ClO₂ service with corrosion-resistant titanium-lined valves. UT thickness test prior to re-commissioning.", owner: maintHead.id, status: "COMPLETED", daysFromDetect: 30 },
        { type: "PREVENTIVE", desc: "Revise preventive maintenance checklist to include UT thickness measurement for all valves in ClO₂ and other corrosive bleach-plant service. Frequency: quarterly.", owner: safetyOfficer.id, status: "COMPLETED", daysFromDetect: 45 },
      ],
      rootCauses: [
        { desc: "6-monthly visual inspection did not include ultrasonic wall thickness measurement on valve stems in corrosive bleach-plant service.", cat: "PROCESS", confidence: "HIGH" },
        { desc: "PM checklist had no specific line item for CI valve stem integrity in ClO₂ dosing service.", cat: "DOCUMENTATION", confidence: "HIGH" },
      ],
    },
    {
      num: "002", severity: "MODERATE", state: "VERIFIED",
      title: "Reel-handling crane wire rope wear — inspection finding CAPA",
      sourceTypeCode: "INSPECTION_FINDING", sourceCatCode: "SAFETY", subCatCode: "EQUIPMENT",
      actionType: "CORRECTIVE_AND_PREVENTIVE",
      problem: "Monthly inspection of the paper machine reel-handling EOT crane found wire rope with 3 broken wires in one lay, exceeding the 2-wire discard criterion. Crane taken out of service immediately.",
    ...
    },
    {
      ...
      actionType: "CORRECTIVE_AND_PREVENTIVE",
      problem: "Electrician began work on the paper machine drive MCC panel without verifying de-energisation. Arc flash occurred; PPE prevented injury. Near-miss escalated to CAPA given high potential severity.",
    ...
    },
    {
      ...
      actionType: "CORRECTIVE_AND_PREVENTIVE",
      problem: "Internal audit (IMS-AUD-2026-Q1) identified that 3 of 5 caustic soda handling operators could not locate the current revision of SOP-CHEM-005. One operator using superseded procedure.",
    ...
    },
    {
      ...
      title: "Housekeeping deficiency — slip hazard in waste-paper baling store",
      sourceTypeCode: "SAFETY_OBSERVATION", sourceCatCode: "SAFETY", subCatCode: "ENVIRONMENTAL",
      actionType: "CORRECTIVE_ONLY",
      problem: "Safety walk identified spilled loose fibre and broke on the access aisle in the waste-paper baling store. Aisle not cleaned at end of shift as per housekeeping standard.",
      ...
      actions: [
        { type: "CORRECTIVE", desc: "Clear spilled fibre and broke; inspect and clean all access aisles in the waste-paper baling store. Verify no other contamination.", owner: supervisor.id, status: "PROPOSED", daysFromDetect: 1 },
        { type: "CORRECTIVE", desc: "Reinforce end-of-shift housekeeping checklist for the baling store in next team briefing. Supervisor to verify completion daily for 30 days.", owner: supervisor.id, status: "PROPOSED", daysFromDetect: 7 },
      ],
      rootCauses: [
    ...
    },
    {
      ...
      title: "HIRA recommended control — high noise at paper machine vacuum pump & blower hall",
      sourceTypeCode: "HIRA_CONTROL", sourceCatCode: "SAFETY", subCatCode: "ENVIRONMENTAL",
      actionType: "PREVENTIVE_ONLY",
      problem: "HIRA review of the paper machine vacuum pump & blower hall identified noise level averaging 95 dB(A) — exceeding the 85 dB action level. Engineering noise control not yet installed. Hearing protection mandatory interim control only.",
      ...
      actions: [
        { type: "PREVENTIVE", desc: "Procure and install acoustic enclosures around vacuum pump units 1–3. Target: reduce noise at operator station to < 80 dB(A). Engineering evaluation to specify design.", owner: maintHead.id, status: "PROPOSED", daysFromDetect: 60 },
      ],
      rootCauses: [],
    },
  ];
  ...
  for (const c of CAPAS) {
    ...
    const capa = await prisma.capa.create({
      data: {
        ...
        affectedAreas: ["Bleaching Plant — ClO₂ Dosing Area", "Paper Machine Vacuum Pump & Blower Hall"],
        affectedProcesses: ["Bleaching", "Equipment Maintenance"],
      },
    });
