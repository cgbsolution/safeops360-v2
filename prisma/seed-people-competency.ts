...
async function seedPlant(plantCode: "NW" | "SW") {
  ...
  const sch1 = await prisma.trainingSchedule.create({
    data: {
      ...
      venue: `${P === "NW" ? "North Paper Mill" : "South Kraft & Board Mill"} — Training Room 1`,
  ...
  const sch2 = await prisma.trainingSchedule.create({
    data: {
      ...
      venue: `${P === "NW" ? "North Paper Mill" : "South Kraft & Board Mill"} — Training Room 2 + Workshop Bay`,
      language: "English", trainerId: trainer.id, isExternalTrainer: false,
      ...
      if (validFrom) {
        await prisma.competencyAssessment.create({
          data: {
            ...
            location: `${P === "NW" ? "North Paper Mill" : "South Kraft & Board Mill"} — Training Centre`,
        ...
        if (["CS-ENTRANT-L1", "HOT-WORK-WELDER-SMAW", "WORK-HEIGHT-L1"].includes(scenario.code)) {
          await prisma.supervisedPerformanceRecord.create({
            data: {
              ...
              activityLocation: `Pulp Mill — ${P === "NW" ? "North Paper Mill" : "South Kraft & Board Mill"}`,
  ...
  await prisma.recertificationCycle.create({
    data: {
      ...
      name: `Q2 2026 Competency Revalidation Cycle — ${P === "NW" ? "North Paper Mill" : "South Kraft & Board Mill"}`,
