...
async function seedKaizen(
  ...
  const posts: KaizenSpec[] = [
    // APPROVED × 3
    {
      ...
      hazardSeveritySelf: "HIGH", locationTag: "Power & Steam Plant — Recovery Boiler Area",
      description: "The walkway alongside Recovery Boiler #2 has a corroded grating section (approx 40 cm × 30 cm) near the expansion joint. In wet conditions the corroded section flexes visibly. I nearly slipped last Thursday during a routine round. Suggested fix: replace the grating section with chequer plate.",
    ...
    },
    {
      ...
      hazardSeveritySelf: "LOW", locationTag: "QC & Pulp/Paper Testing Laboratory",
      description: "The QC Lab team introduced a colour-coded sample rack system where all pulp and black-liquor samples requiring fume hood handling are placed in RED racks. This has eliminated the previous confusion about which samples needed ventilated handling and has improved throughput. Recommend rolling out to the SW lab.",
    ...
    // PENDING_COMMITTEE × 2
    {
      ...
      hazardSeveritySelf: "MEDIUM", locationTag: "Reel & Ream Warehouse — Reel Dispatch",
      description: "The reversing alarm on reel-handling Forklift FLT-003 is much quieter than the other forklifts. In the noisy reel dispatch area, pedestrians moving between stacked paper reels cannot hear it until the vehicle is very close. The alarm should be replaced or the sensitivity increased. Multiple colleagues have mentioned this to me.",
      status: "PENDING_COMMITTEE",
      createdAt: daysAgo(3),
    },
    {
      submitterUserId: envMgr, isAnonymous: true, category: "UNSAFE_ACT",
      hazardSeveritySelf: "HIGH", locationTag: "Paper Machine Hall — Dry-end hot work bay",
      description: "On two separate occasions this month I observed arc welding being carried out near the paper machine dry end without a fire watch person being present, as required by the hot work permit. The welder was working alone. When I asked, he said the fire watch had 'stepped out for a minute.' This is a permit violation and a significant fire risk given the dry broke and paper dust accumulation nearby.",
      status: "PENDING_COMMITTEE",
      createdAt: daysAgo(1),
    },
    // DECLINED × 1
