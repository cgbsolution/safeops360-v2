...
//
// Seeds:
//   • Industry checkpoint libraries: PULP_PAPER (full, ~100 checkpoints
...
const GARMENTS_CATEGORIES: Category[] = [
  {
    ...
    checkpoints: [
      ...
      cp({ code: "GT-FS-008", question: "Is storage of finished paper reels, waste-paper bales and other combustible materials compliant with spacing and stacking requirements?", criticality: "critical" }),
  ...
  },
  {
    ...
    sequence: 3,
    checkpoints: [
      cp({ code: "GT-MS-001", question: "Are all machine guards (nip-point guards, roll guards, coupling and belt covers) in place and functional on the paper machines and refiners?", requirement_reference: "Factories Act §21-27, ISO 45001 Cl.8.1", criticality: "critical" }),
      cp({ code: "GT-MS-002", question: "Are emergency stop buttons and pull-cords accessible and functional along the paper machine, winders and conveyors?", criticality: "critical" }),
      cp({ code: "GT-MS-003", question: "Are sheet cutters and rewinder/slitter knives equipped with anti-contact devices and is cut-resistant PPE provided?", criticality: "critical" }),
      ...
      cp({ code: "GT-MS-006", question: "Are the power boiler and recovery boiler certified and inspected per the Indian Boilers Act (IBR)?", criticality: "critical" }),
      cp({ code: "GT-MS-007", question: "Are dryer-section cylinders, pulpers and stock-chest agitators free from exposed moving parts without guards?", criticality: "major" }),
      ...
      cp({ code: "GT-MS-009", question: "Are broke-handling and sheet-break clearance procedures followed safely at the paper machine?", criticality: "major" }),
      ...
      cp({ code: "GT-MS-011", question: "Are bale presses and reel-handling clamps fitted with two-hand controls / interlocked guards?", criticality: "major" }),
    ],
  },
  {
  ...
  },
  {
    ...
    checkpoints: [
      ...
      cp({ code: "GT-CH-002", question: "Are caustic soda, chlorine dioxide, sizing agents and other process chemicals stored in designated areas with secondary containment?", criticality: "major" }),
  ...
  },
  {
    ...
    checkpoints: [
      ...
      cp({ code: "GT-PP-005", question: "Are eye-wash stations and safety showers functional and accessible in the bleach plant and chemical-handling areas?", criticality: "major" }),
      cp({ code: "GT-PP-006", question: "Are cut-resistant gloves provided and used at sheet-cutting and rewinder/slitter workstations?", linked_safeops_module: "ppe", criticality: "major" }),
    ],
  },
  {
    ...
    sequence: 7,
    checkpoints: [
      cp({ code: "GT-HK-001", question: "Are all walkways and aisles free from paper reels, broke, pulp bales, and obstructions?", criticality: "major" }),
      ...
      cp({ code: "GT-HK-003", question: "Is ventilation adequate in the dryer section and Yankee/MG hood areas — no heat-stress risk?", criticality: "major" }),
      ...
      cp({ code: "GT-HK-005", question: "Is paper-dust and fibre accumulation controlled in the finishing house and stock-preparation areas?", criticality: "major" }),
      ...
      cp({ code: "GT-HK-007", question: "Is broke / paper trim collected in designated bins or broke pulpers and cleared each shift?", criticality: "minor" }),
    ],
  },
  {
...
async function main() {
  ...
  await prisma.auditCheckpointLibrary.createMany({
    data: [
      { industryCode: "GARMENTS_TEXTILE", industryName: "Pulp & Paper Manufacturing", version: "2026.1", categories: GARMENTS_CATEGORIES as any, checkpointCount: countCheckpoints(GARMENTS_CATEGORIES) },
  ...
  const tFull = await prisma.auditTemplate.create({
    data: {
      name: "SA8000 + ISO 45001 Integrated Audit — Pulp & Paper", description: "Full integrated social-compliance + OH&S audit across all 10 categories.",
  ...
  await prisma.auditTemplate.create({
    data: {
      name: "Fire Safety Focused Audit — Pulp & Paper", description: "Rapid fire & emergency-preparedness audit.",
  ...
  await prisma.auditTemplate.create({
    data: {
      name: "Worker Welfare & Social Compliance — Pulp & Paper", description: "SA8000-aligned worker welfare audit.",
  ...
  async function makeAudit(opts: {
    ...
    const audit = await prisma.complianceAudit.create({
      data: {
        ...
        scopeDepartments: ["Pulp Mill", "Paper Machine", "Finishing", "Stores"] as any, scopeAreas: ["Paper Machine Hall", "Bleaching Plant"] as any,
  ...
  await makeAudit({ seq: 1, plantId: nw.id, title: "Q2 Integrated Audit — North Paper Mill (In Progress)", status: "in_progress", scheduledDate: new Date("2026-06-09T03:30:00.000Z"), answeredFraction: 0.55, computeFinal: false });
  await makeAudit({ seq: 2, plantId: nw.id, title: "SA8000 Mid-Year Audit — North Paper Mill (Awaiting Responses)", status: "submitted_pending_response", scheduledDate: new Date("2026-06-02T03:30:00.000Z"), answeredFraction: 1.0, computeFinal: true, submitted: true });
  await makeAudit({ seq: 3, plantId: nw.id, title: "Q1 Integrated Audit — North Paper Mill (Closed)", status: "closed", scheduledDate: new Date("2026-03-12T03:30:00.000Z"), answeredFraction: 1.0, computeFinal: true, closed: true });
  if (sw) {
    await makeAudit({ seq: 1, plantId: sw.id, title: "Q2 Integrated Audit — South Kraft & Board Mill (Scheduled)", status: "scheduled", scheduledDate: new Date("2026-06-20T03:30:00.000Z"), answeredFraction: 0, computeFinal: false });
  }
