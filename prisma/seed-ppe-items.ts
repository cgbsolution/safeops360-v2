...
async function seedPlant(plantId: string, code: "NW" | "SW", hseMgr: string, worker: string, deptHead: string, types: PpeTypeIds) {
  ...
  const itemSpecs: ItemSpec[] = [
    ...
    { itemNumber: `DEMO-${code}-GASDET-001`, serial: `BW-4G-${code}-001`, typeId: TYPE_GAS_DET, typeCode: "GAS-DETECTOR-4GAS", typeName: "Portable 4-Gas Detector", mfr: "BW Technologies (Honeywell)", model: "GasAlertMicro5 IR", batch: `GD-STD-2022`, mfgDate: new Date("2022-02-01"), purchaseDate: new Date("2022-03-10"), cost: 28000, status: "issued", condition: "fair", storageLocation: `${code} Pulp Mill Control Room`, currentHolderUserId: deptHead, issuedSince: daysAgo(180) },
