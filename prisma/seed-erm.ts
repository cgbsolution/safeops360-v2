...
// (plants NW = North Garment Unit, SW = South Garment Unit — already created by seed.ts):
...
async function main() {
  ...
  const personas: PersonaSpec[] = [
    ...
    { name: "Vikram Sethi", email: "vikram.sethi@safeops360.in", roleCode: "RISK_CHAMPION", plantId: sw.id, designation: "Risk Champion (South Garment Unit)" },
    ...
    { name: "Devendra Kulkarni", email: "devendra.kulkarni@safeops360.in", roleCode: "PLANT_HSE_HEAD", plantId: nw.id, designation: "Plant HSE Head (North Garment Unit)" },
    { name: "Lakshmi Venkatesh", email: "lakshmi.venkatesh@safeops360.in", roleCode: "PLANT_HSE_HEAD", plantId: sw.id, designation: "Plant HSE Head (South Garment Unit)" },
  ...
  const ruleNW = await prisma.rollupRule.create({
    data: {
      name: "North Garment Unit High-Band HSE Rollup",
  ...
  const ruleSW = await prisma.rollupRule.create({
    data: {
      name: "South Garment Unit High-Band HSE Rollup",
  ...
  const RISKS: RiskSpec[] = [
    ...
    { code: "0003", title: "South Garment Unit expansion cost overrun", subCode: "STR-EXP", il: 3, ii: 4, rl: 3, ri: 3, state: "TREATMENT_ACTIVE", owner: "Rajesh N", velocity: "MODERATE", orgLevel: "SITE", plant: "SW", sourceType: "MANUAL", dominantDim: "FINANCIAL", bumpPrior: true },
    ...
    { code: "0006", title: "Raw material price escalation (cotton, yarn imports)", subCode: "FIN-CMD", il: 5, ii: 3, rl: 4, ri: 3, state: "TREATMENT_ACTIVE", owner: "Meera I", velocity: "FAST", orgLevel: "ENTERPRISE", plant: null, sourceType: "MANUAL", dominantDim: "FINANCIAL", bumpPrior: true },
    ...
    { code: "0008", title: "Aggregated HSE risk — North Garment Unit (rollup)", subCode: "OPS-HSE", il: 4, ii: 4, rl: 3, ri: 4, state: "MONITORING", owner: "Devendra K", velocity: "FAST", orgLevel: "SITE", plant: "NW", sourceType: "HSE_ROLLUP", rollupRuleId: ruleNW.id, dominantDim: "SAFETY", bumpPrior: true },
    { code: "0009", title: "Aggregated HSE risk — South Garment Unit (rollup)", subCode: "OPS-HSE", il: 4, ii: 3, rl: 3, ri: 3, state: "MONITORING", owner: "Lakshmi V", velocity: "FAST", orgLevel: "SITE", plant: "SW", sourceType: "HSE_ROLLUP", rollupRuleId: ruleSW.id, dominantDim: "SAFETY" },
    ...
    { code: "0011", title: "Single-line dependency — North Garment Unit Dye House Line 3", subCode: "OPS-AST", il: 3, ii: 5, rl: 2, ri: 5, state: "TREATMENT_ACTIVE", owner: "Devendra K", velocity: "VERY_FAST", orgLevel: "SITE", plant: "NW", sourceType: "MANUAL", dominantDim: "BUSINESS_INTERRUPTION", bumpPrior: true },
    { code: "0012", title: "Quality escape to key retail / e-commerce customer", subCode: "OPS-QUA", il: 3, ii: 4, rl: 2, ri: 4, state: "MONITORING", owner: "Lakshmi V", velocity: "FAST", orgLevel: "ENTERPRISE", plant: null, sourceType: "MANUAL", dominantDim: "REPUTATIONAL" },
    ...
    { code: "0015", title: "CPCB consent renewal delay — South Garment Unit ETP", subCode: "CMP-LIC", il: 3, ii: 5, rl: 2, ri: 5, state: "TREATMENT_ACTIVE", owner: "Lakshmi V", velocity: "MODERATE", orgLevel: "SITE", plant: "SW", sourceType: "MANUAL", dominantDim: "REGULATORY", bumpPrior: true },
    { code: "0016", title: "Community grievance — North Garment Unit night logistics", subCode: "REP-COM", il: 3, ii: 3, rl: 2, ri: 3, state: "MONITORING", owner: "Suresh P", velocity: "MODERATE", orgLevel: "ENTERPRISE", plant: null, sourceType: "MANUAL", dominantDim: "REPUTATIONAL", overdue: true },
    { code: "0017", title: "Jockey / Speedo brand-licence dependency & renewal risk", subCode: "REP-BRD", il: 2, ii: 4, rl: 2, ri: 3, state: "MONITORING", owner: "Meera I", velocity: "FAST", orgLevel: "ENTERPRISE", plant: null, sourceType: "MANUAL", dominantDim: "REPUTATIONAL" },
    ...
    { code: "0021", title: "Water stress — South Garment Unit groundwater dependency", subCode: "ESG-PHY", il: 4, ii: 4, rl: 3, ri: 4, state: "TREATMENT_ACTIVE", owner: "Lakshmi V", velocity: "SLOW", orgLevel: "SITE", plant: "SW", sourceType: "MANUAL", dominantDim: "BUSINESS_INTERRUPTION", bumpPrior: true },
    { code: "0022", title: "Single-source dependency — dye & chemical vendor", subCode: "SCM-SRC", il: 4, ii: 4, rl: 3, ri: 4, state: "TREATMENT_ACTIVE", owner: "Meera I", velocity: "FAST", orgLevel: "ENTERPRISE", plant: null, sourceType: "MANUAL", dominantDim: "BUSINESS_INTERRUPTION", bumpPrior: true },
  ...
  const narrative: Record<string, { desc: string; causes: string[]; consequences: string[]; controls: string[]; tags: string[] }> = {
    ...
    "0006": { desc: "Volatility in cotton and imported-yarn prices (cause) raises raw-material cost faster than selling prices can be reset (event), compressing gross margin (consequence).", causes: ["Commodity price spikes", "Pass-through lag in contracts", "Supplier price hikes"], consequences: ["Gross-margin erosion", "Quote competitiveness loss", "Inventory valuation swings"], controls: ["Index-linked pricing clauses", "Strategic raw-material inventory"], tags: ["commodity", "margin"] },
    ...
    "0011": { desc: "Critical dependency on a single dye-house line at North Garment Unit (cause) means an extended Dye House Line 3 breakdown (event) would halt a major product family and breach customer commitments (consequence).", causes: ["No redundant line", "Aging critical equipment", "Long spares lead time"], consequences: ["Extended production halt", "Customer SLA breach", "Revenue loss"], controls: ["Preventive maintenance plan", "Critical-spares holding"], tags: ["asset", "single-point", "north-works"] },
    "0012": { desc: "An undetected garment quality defect (cause) escaping to a key retail / e-commerce customer (event) could trigger a lot rejection / chargeback claim and damage the relationship (consequence).", causes: ["Inspection escape", "Process drift", "Inadequate AQL control"], consequences: ["Customer chargeback claim", "Reputational damage", "Lot rejection cost"], controls: ["SPC at critical stations", "Final AQL inspection gates"], tags: ["quality", "customer"] },
    ...
    "0015": { desc: "Delay in renewing the CPCB consent-to-operate for the South Garment Unit ETP (cause) could leave the plant operating without valid consent (event), exposing it to a closure direction (consequence).", causes: ["Renewal application delay", "Pending compliance conditions", "Regulatory backlog"], consequences: ["Operation without consent", "Closure direction", "Penalty"], controls: ["Consent-renewal tracker", "Pre-emptive compliance closure"], tags: ["compliance", "environment", "south-works"] },
    "0016": { desc: "Night-time logistics movements near a residential cluster at North Garment Unit (cause) generate noise and traffic grievances (event), risking community escalation and adverse publicity (consequence).", causes: ["Night-shift truck movements", "Noise near residences", "Limited grievance redress"], consequences: ["Community protest", "Local media coverage", "Operating-hour restriction"], controls: ["Community grievance mechanism", "Routing & timing controls"], tags: ["community", "north-works"] },
    "0017": { desc: "The business depends on the exclusive Jockey and Speedo brand licences for India (cause); an adverse change in licensor terms or a non-renewal at expiry (event) would remove the core revenue franchise and brand equity (consequence).", causes: ["Licensor strategy / ownership change", "Royalty or territory renegotiation", "Brand-standard or compliance shortfall"], consequences: ["Loss of brand franchise", "Revenue and valuation shock", "Channel and EBO disruption"], controls: ["Licensor relationship & performance governance", "Brand-standard and compliance assurance programme"], tags: ["brand", "licence"] },
    ...
    "0021": { desc: "Over-reliance on declining groundwater at South Garment Unit (cause) during a drought year (event) could force production curtailment and invite regulatory limits (consequence).", causes: ["Declining water table", "Drought conditions", "High specific water use"], consequences: ["Production curtailment", "Abstraction limits", "Higher water cost"], controls: ["Water recycling & ZLD roadmap", "Rainwater harvesting"], tags: ["climate", "water", "south-works"] },
    "0022": { desc: "Dependence on a single qualified vendor for reactive dyes & dye-house chemicals (cause) means a vendor disruption (event) would stop production of dependent dyed SKUs (consequence).", causes: ["Single qualified source", "No alternate qualification", "Vendor financial stress"], consequences: ["Input stock-out", "Production halt for dependent SKUs", "Expedite premiums"], controls: ["Buffer inventory", "Alternate-vendor qualification (in progress)"], tags: ["supply-chain", "single-source"] },
  ...
  const linkages: { src: string; tgt: string; type: string; notes: string }[] = [
    ...
    { src: "0022", tgt: "0011", type: "TRIGGERS", notes: "A single-source dye-chemical disruption can force a stoppage on the dependent North Garment Unit dye-house line." },
  ...
  const CAPAS: CapaSpec[] = [
    { riskCode: "0003", strategy: "TREAT", title: "Tighten South Garment Unit expansion cost controls", problem: "Project trending above sanctioned budget; reinforce stage-gate cost governance and change control.", state: "ACTIONS_IN_PROGRESS", severity: "HIGH", priority: "HIGH", expectedReduction: "Hold residual at MEDIUM by capping overrun" },
    ...
    { riskCode: "0006", strategy: "TREAT", title: "Build strategic raw-material buffer stock", problem: "Price-spike exposure on cotton/imported yarn; establish a strategic buffer-inventory policy.", state: "ACTIONS_PLANNED", severity: "HIGH", priority: "HIGH", expectedReduction: "Smooth price shocks; residual MEDIUM" },
    { riskCode: "0011", strategy: "TRANSFER", title: "Place machinery breakdown insurance on Dye House Line 3", problem: "Single-line dependency at North Garment Unit Dye House Line 3; transfer breakdown loss via insurance placement.", state: "PENDING_VERIFICATION", severity: "HIGH", priority: "HIGH", expectedReduction: "Transfer financial loss; residual HIGH" },
    ...
    { riskCode: "0015", strategy: "TREAT", title: "Expedite South Garment Unit ETP consent renewal", problem: "CPCB consent renewal lagging; close pending conditions and expedite the renewal application.", state: "ACTIONS_PLANNED", severity: "HIGH", priority: "HIGH", expectedReduction: "Restore valid consent; residual HIGH" },
    ...
    { riskCode: "0021", strategy: "TREAT", title: "Commission water recycling & rainwater harvesting", problem: "Groundwater dependency at South Garment Unit; reduce abstraction via recycling and harvesting.", state: "ACTIONS_PLANNED", severity: "HIGH", priority: "HIGH", expectedReduction: "Cut freshwater draw; residual HIGH" },
    { riskCode: "0022", strategy: "TERMINATE", title: "Qualify alternate dye & chemical vendor", problem: "Single-source dye-chemical dependency; qualify an alternate vendor to terminate the single point of failure.", state: "ACTIONS_IN_PROGRESS", severity: "HIGH", priority: "HIGH", expectedReduction: "Eliminate single source; residual MEDIUM" },
  ...
  await prisma.ermBoardPack.create({
    data: {
      ...
      commentary: {
        ...
        [riskIdByCode.get("0011")!]: "<p>Single-line dependency at North Garment Unit Dye House Line 3 carries the highest residual impact; insurance placement pending verification.</p>",
      } as any,
      createdBy: croId,
    },
  });
